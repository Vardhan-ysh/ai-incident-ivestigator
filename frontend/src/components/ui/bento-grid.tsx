...
const BentoCard = ({
  ...
  <div
    ...
    className={cn(
      ...
      "bg-neutral-900 border border-neutral-800 [box-shadow:0_0_0_1px_rgba(255,255,255,.03),0_2px_4px_rgba(0,0,0,.4),0_12px_24px_rgba(0,0,0,.4)]",
      className,
    )}
  >
    {/* Background fills the card absolutely so it never pushes text out */}
    <div className="absolute inset-0">{background}</div>

    {/* Scrim: fades the bottom half to card-bg so text is always readable */}
    ...
    >
      <Button variant="ghost" asChild size="sm" className="pointer-events-auto text-neutral-300 hover:text-white hover:bg-neutral-800">
        <a href={href}>
          {cta}
